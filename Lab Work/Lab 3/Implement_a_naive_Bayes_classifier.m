...
% Bayes classification done inside this for loop
for i = 1:(testingSamples * 2)
    result = BayesClassifier(testingData(1,i),testingData(2,i),c1mean,c2mean,newSigma);
    if(result == 1)
        resClassA = [resClassA, testingData(:,i)];
    else
        resClassB = [resClassB, testingData(:,i)];
    end      
end

figure 
hold on
plot(resClassA(1,:), resClassA(2,:), 'ro');
plot(resClassB(1,:), resClassB(2,:), 'b+');
xlabel('x-dimension');
ylabel('y-dimension');
